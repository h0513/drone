...
def connect(vb: bool=False):
    if vb: out.inf("initiating connection to drone")
    global api
    api = hula.UserApi()
    ckcon(True, True)
    if ckcon():
        if vb: out.suc("connected to drone")
        return 1
    else: return 0
...
def preflight(ins=True, vb=True):
    if not ckcon():
        return 0
...
def light(pset="-1", t=0, r=0, g=0, b=0, md=1, vb=False):
    if not pset == "-1":
...
def tfinit(model, label):
    ...
    if label[:5] == "path/":
        label = r"C:/Users/34/Desktop/drone/model/" + label[5:]
