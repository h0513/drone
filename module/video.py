...
class hula_video:
    def __init__(self,hula_api,hula_ip="0.0.0.0",display=False):
        ...
        self.buffer_size = 25
        self.savepath = os.path.join(os.getcwd(),'image')
        if not os.path.exists(self.savepath):
            out.inf("creating save directory image")
    ...
    def video_mode_on(self):
        ...
        while self.lib.get_rgb_datas_length() <= 0:
            pass
        out.suc('video stream started')
        self.videothread.start()

    def start_recording(self,filename="image"):
        '''
    ...
    def _receive_video_data(self):
        out.suc(f'started video receiver for drone {self.video_port}')
