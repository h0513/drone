...
class vid:
    def __init__(self,api,hula_ip="0.0.0.0",display=False):
        self.uapi = api
        ...
        if not os.path.exists(self.savepath):
            out.inf("creating save directory")
    ...
    def vo(self):
        ...
        out.inf("starting video stream")
        time.sleep(1)
        ret = self.lib.init_video(self.hula_ip,self.video_port)
        while self.lib.get_rgb_datas_length() <= 0:
            pass
        out.suc('completed video stream startup')
        self.videothread.start()

    def sr(self,filename="photo"):
        self.record = True
    ...
    def gv(self,get_latest=True,keep_getting=True):
    ...
    def _receive_video_data(self):
        out.inf(f"started video receiver for drone {self.video_port}")
